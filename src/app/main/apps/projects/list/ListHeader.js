...
import { setProjectsSearchText, selectProjects, getAllProjects } from '../store/projectsSlice';
...
function ListHeader(props) {
  ...
  function handleExcelExport() {
    dispatch(getAllProjects()).then((action) => {
      const listItems = action.payload.map(function (p) {
        return mapObj(p);
      });
      exportFromJSON({ data: listItems, fileName: 'Projects', exportType: 'xls' });
    });
  }
  ...
  function handleCSVExport() {
    dispatch(getAllProjects()).then((action) => {
      ...
      exportFromJSON({ data: listItems, fileName: 'Projects', exportType: 'csv' });
    });
  }
  ...
  return (
    <div className="flex flex-1 w-full items-center justify-between">
      ...
      >
        {permissions.indexOf("projects:create") > -1 && <Button
          component={Link}
          ...
          color="secondary"
        >
          <span className="hidden sm:flex">Add Projects</span>
