...
import { getMember, getAllParentLevels, getDynamicForms } from '../store/memberSlice';
...
function Details() {
  ...
  useEffect(() => {
    dispatch(getMember(routeParams)).then((action) => {
      ...
      dispatch(getAllParentLevels({"parent_id": action.payload.level_data_id})).then((action) => {
        let listdata = [];
