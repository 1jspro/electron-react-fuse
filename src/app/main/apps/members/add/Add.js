...
import { getAllGroup } from '../../group/store/GroupDataListSlice';
...
const schema = yup.object().shape({
  ...
  email: yup.string().nullable().email('You must enter a valid email'),
  phone_no: yup
    .string()
  ...
  policy_number: yup
    .string()
    .nullable()
    .matches(/^[aA-zZ0-9\s]*$/, 'Only alphanumeric characters are allowed for this field '),
});
...
function Member(props) {
  ...
  useEffect(() => {
    dispatch(getLevels()).then((action) => {
      ...
      dispatch(getLevelsData({ level_id: top_level_id })).then((action) => {
        ...
        /* if(datalist[top_level_id].length === 1){
          setValue('level_' + top_level_id,datalist[top_level_id][0].id+'')
        } */
      });
    });
    ...
    function updateMemberState() {
      ...
      if (memberId === 'new') {
        ...
        dispatch(getMember(routeParams)).then((action) => {
          /**
           * If the requested member is not exist show message
           */
          if (!action.payload) {
            setNoMember(true);
          }
          ...
          if (action.payload.extra_field) {
            const extraField = JSON.parse(action.payload.extra_field);
            setExtraFields(extraField);
            setValue('extra_field', extraField);
          }
          if (action.payload.groups) {
            setValue('group', action.payload.groups);
          }
  ...
  function handleLevelChange(key, value) {
    ...
    if (levels && key < levels.length) {
      ...
      if (level_id) {
        dispatch(getLevelsData({ level_id, parent_id: value })).then((action) => {
          const datalist = [...levelDataList];
   ...
   */
  const isNotFormReady =
    _.isEmpty(form) ||
    (member && routeParams.memberId !== member.encryption_id && routeParams.memberId !== 'new') ||
    !items.length;

  function handleOnDefaultFormMod(formArr) {
    setItems(formArr);
  }
  ...
  return (
    <FormProvider {...methods}>
      <Root
        header={<PageHeader onDefaultFormMod={handleOnDefaultFormMod} />}
        content={
          <>
            {!isNotFormReady ? (
