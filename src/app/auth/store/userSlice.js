...
export const saveMirrorUser =
  ({ access_token }) =>
  async (dispatch, getState) => {
    const oldUserToken = window.localStorage.getItem("jwt_access_token");

    localStorage.setItem("muid", oldUserToken);
    localStorage.setItem("jwt_access_token", access_token);
    axios.defaults.headers.common.Authorization = `Bearer ${access_token}`;
    // return dispatch(setUserData(user));
  };

export const exitMirrorUser =
  ({}) =>
  async (dispatch, getState) => {
    const oldUserToken = window.localStorage.getItem("muid");
    localStorage.setItem("jwt_access_token", oldUserToken);
    localStorage.setItem("muid", "");
    axios.defaults.headers.common.Authorization = `Bearer ${oldUserToken}`;
    // return dispatch(setUserData(user));
  };
