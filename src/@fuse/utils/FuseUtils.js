...
class FuseUtils {
  ...
  static setRoutes(config, defaultAuth) {
    const permissionsStr = window.localStorage.getItem('permissions');
    const permissions = permissionsStr ? permissionsStr.split(",") : []; 
    let routes = [...config.routes];
    routes = config.routes.filter((route) => {
      let permission = route.permission || route.permission === null ? route.permission : null;
      if (!permission || (permission && permissions.indexOf(permission) !== -1)) {
        return route;
      }
    }).map((route) => {
      let meta = config.meta || config.meta === null ? config.meta : null;
