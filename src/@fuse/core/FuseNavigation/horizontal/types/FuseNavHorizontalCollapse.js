...
function FuseNavHorizontalCollapse(props) {
  ...
  if (item && item.children && item.children.length > 0) {
    item.children.forEach(function(c) {
      if (!c.meta || (c.meta && c.meta.permission && permissions.indexOf(c.meta.permission) !== -1)) {
        allowNav = true;
      }
    });
  } else if (!item.meta || (item.meta && item.meta.permission && permissions.indexOf(item.meta.permission) !== -1)) {
    allowNav = true;
  }
        ...
        <ul className="relative px-0">
          <Manager>
            ...
            {ReactDOM.createPortal(
              <Popper
                placement={theme.direction === 'ltr' ? 'bottom-start' : 'bottom-end'}
